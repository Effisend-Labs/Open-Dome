...
const WALLET_CIRCLE_TOOLS = exports.WALLET_CIRCLE_TOOLS = [{
  functionDeclarations: [{
    ...
    description: 'Estimate fee for a USDC transfer. Sponsored L2s (BASE/ARB/OP/MATIC/AVAX) report userFee 0. ETH and SOL are user-paid gas.',
    parameters: {
      type: 'OBJECT',
      properties: {
        walletId: {
    ...
    description: 'Send USDC. Pass blockchain for the source chain. L2s are gasless (facilitator). ETH/SOL require native gas. Solana dest from Base uses CCTP.',
    parameters: {
      type: 'OBJECT',
      properties: {
        amount: {
