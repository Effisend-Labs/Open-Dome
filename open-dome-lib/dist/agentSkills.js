...
exports.WALLET_CIRCLE_TOOLS = exports.WALLET_CIRCLE_PROMPT = exports.GOOGLE_SEARCH_TOOLS = exports.DOME_CONSULTANT_TOOLS = exports.DOME_CONSULTANT_PROMPT = void 0;
exports.resolveAgentMode = resolveAgentMode;
exports.runDomeConsultantTool = runDomeConsultantTool;
var _events = require("./events.js");
var _itinerary = require("./itinerary.js");
var _planner = require("./planner.js");
const GOOGLE_SEARCH_TOOLS = exports.GOOGLE_SEARCH_TOOLS = [{
  googleSearch: {}
}];
  ...
  functionDeclarations: [{
    ...
    parameters: {
      ...
      properties: {
        ...
        placeName: {
          type: 'STRING',
          description: 'e.g. Tokyo Dome, Korakuen Hall, IMM Theater'
        },
        category: {
          type: 'STRING'
        },
...
const WALLET_CIRCLE_PROMPT = exports.WALLET_CIRCLE_PROMPT = `You are the OpenDome Wallet agent. You operate Circle developer-controlled wallets for the signed-in user.
...
const WALLET_CIRCLE_TOOLS = exports.WALLET_CIRCLE_TOOLS = [{
  functionDeclarations: [{
    name: 'list_wallets',
    description: 'List the signed-in user Circle wallets with token balances on every chain (Base, Arb, OP, Polygon, Avalanche, Ethereum, Solana). Each row includes usdc.',
    parameters: {
      type: 'OBJECT',
      properties: {
        blockchain: {
    ...
    description: 'Token balances for a wallet. Omit walletId and blockchain to fetch USDC on all user chains. Pass blockchain (BASE/ARB/OP/MATIC/AVAX/ETH/SOL) when walletId is unknown.',
    parameters: {
      type: 'OBJECT',
      properties: {
        walletId: {
    ...
    description: 'Estimate fee for a USDC transfer. Sponsored L2s and Solana report userFee 0. Ethereum is user-paid gas.',
    parameters: {
      type: 'OBJECT',
      properties: {
        walletId: {
    ...
    description: 'Send USDC. Pass blockchain for the source chain. L2s and Solana are gasless through facilitators. Ethereum requires native gas. Solana dest from Base uses CCTP.',
    parameters: {
      type: 'OBJECT',
      properties: {
        amount: {
...
function runDomeConsultantTool(name, args = {}) {
  if (name === 'search_events') {
    ...
    const rows = args.query || args.category ? _events.Events.search({
      query: args.query,
      placeName: args.placeName,
      category: args.category,
      limit: args.limit || 8
    }) : (0, _planner.listUpcomingEvents)({
      placeName,
      limit: args.limit || 5
    });
    return {
  ...
  if (name === 'get_event') {
    return {
      event: summarizeEvent(_events.Events.getById(args.id))
    };
  }
  if (name === 'list_places') {
    const fromEvents = _events.Events.getAll().map(e => e.placeName).filter(Boolean);
    const fromAmenities = (0, _itinerary.getAmenities)().map(a => a.placeName || a.name);
    return {
      places: [...new Set([...fromEvents, ...fromAmenities])]
    };
  }
  ...
  if (name === 'plan_day') {
    ...
    if (!proposal) return {
      error: 'Could not build a day plan'
    };
    return {
  ...
  return {
    error: `Unknown consultant tool: ${name}`
  };
}
function resolveAgentMode(body = {}) {
