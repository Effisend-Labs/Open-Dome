...
const WALLET_CIRCLE_TOOLS = exports.WALLET_CIRCLE_TOOLS = [{
  functionDeclarations: [{
    ...
    description: 'Estimate fee for a USDC transfer. USDC on Base is facilitator-sponsored (userFee is 0).',
    parameters: {
      type: 'OBJECT',
      properties: {
        walletId: {
    ...
    description: 'Send USDC. Destination may be a Base 0x address (same-chain, gasless) or a Solana address (CCTP bridge from Base USDC).',
    parameters: {
      type: 'OBJECT',
      properties: {
        amount: {
