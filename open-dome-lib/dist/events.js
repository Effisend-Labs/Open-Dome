...
exports.EventsAPI = exports.Events = void 0;
var _events = _interopRequireDefault(require("./dbs/events.json"));
...
class EventsAPI {
  ...
  search(criteria = {}) {
    ...
    if (query) {
      ...
      results = results.filter(event => event.title && event.title.toLowerCase().includes(lowerQuery) || event.title_ja && event.title_ja.toLowerCase().includes(lowerQuery) || event.placeName && event.placeName.toLowerCase().includes(lowerQuery) || event.category && event.category.toLowerCase().includes(lowerQuery));
    }
    if (category) {
      const lowerCat = category.toLowerCase();
      results = results.filter(event => event.category && event.category.toLowerCase() === lowerCat);
    }
    if (placeName) {
      const lowerPlace = placeName.toLowerCase();
      results = results.filter(event => event.placeName && event.placeName.toLowerCase() === lowerPlace);
    }
    if (from !== undefined) {
...
const Events = exports.Events = new EventsAPI();