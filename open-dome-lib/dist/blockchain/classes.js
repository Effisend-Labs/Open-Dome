...
exports.Wallet = exports.TransferToken = exports.Transfer = exports.Blockchain = void 0;
var allChains = _interopRequireWildcard(require("viem/chains"));
...
class Blockchain {
  constructor(options = {
    ...
    this.adapters = {
      solana: new _solana.SolanaAdapter(["https://public.rpc.solanavibestation.com", "https://solana.api.pocket.network", "https://solana.rpc.laine.co"]),
      sol: null,
      // mapped later
      starknet: new _starknet.StarknetAdapter(["https://starknet-rpc.publicnode.com", "https://rpc.starknet.lava.build", "https://api.cartridge.gg/x/starknet/mainnet"])
    };
    ...
    evmList.forEach(chainInput => {
      ...
      if (typeof chainInput === 'string') {
        ...
        chainObj = allChains[chainInput] || Object.values(allChains).find(c => c.name.toLowerCase() === key || c.network === key);
      ...
      if (chainObj) {
        ...
        const adapter = new _evm.EVMAdapter(chainObj);
...
class Transfer {
  ...
  async toObject() {
    ...
    return {
      chain: this.chain,
      amount: this.amount,
      // SDK handles zeros/decimals here
...
class TransferToken {
  ...
  async toObject() {
    const decimals = await (0, _utils.getTokenDecimals)(this.chain, this.tokenAddress);
    return {
      chain: this.chain,
      amount: this.amount,
