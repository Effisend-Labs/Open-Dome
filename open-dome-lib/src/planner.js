...
import { buildItineraryProposal } from './itinerary';

/** Default demo venue — has live upcoming dates in the catalog (Korakuen often lags). */
export const DEFAULT_EVENT_VENUE = 'Tokyo Dome';

/**
 * Pick a venue from free text. Falls back to Tokyo Dome (real upcoming events).
 */
export function resolveVenueFromMessage(text, fallback = DEFAULT_EVENT_VENUE) {
 ...
 */
export function listUpcomingEvents({ placeName = DEFAULT_EVENT_VENUE, limit = 5 } = {}) {
  const now = Date.now();
  // No search limit — need the full venue set to pick the true "next" or "latest"
  const atVenue = Events.search({ placeName }).sort(
...
export function formatEventLine(event, index) {
...
export function formatEventsList(events, placeName) {
  if (!events.length) {
...
export function isEventsCatalogLag(events) {
  if (!events?.length) return false;
  const now = Date.now();
  return events.every((e) => (e.from || 0) < now - 86400000);
}

export function isEventsListIntent(text) {
  return /\b(next|upcoming|which|what|list|show|any)\b.*\b(event|show|concert|game)s?\b/i.test(text)
    || /\b(tokyo\s*dome|korakuen\s*hall|imm\s*theater)\b.*\b(event|show|concert|game)s?\b/i.test(text)
    || /\bevents?\s+at\s+(tokyo\s*dome|korakuen|imm)/i.test(text)
    || /\b(dome|giants|baseball)\b.*\b(event|show|game|schedule)s?\b/i.test(text);
}
...
export function isPurchaseIntent(text) {
  return /\b(buy|book|reserve|checkout|purchase|get tickets|pay|sign)\b/i.test(text)
    && /\b(ticket|pass|itinerary|hours?|slot|reservation|all|everything)\b/i.test(text)
    || /\b(buy the tickets and reserve|reserve that hours?|ok buy)\b/i.test(text);
}

export function pickEventFromMessage(text, session) {
...
export function buildItineraryForEvent(event, { userText } = {}) {
  if (!event) return null;

  const proposal = buildItineraryProposal({
