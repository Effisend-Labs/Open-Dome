...
export const LOCAL_DOCKING_HOST_URL = `http://localhost:${LOCAL_EXPO_PORTS.OpenDomeApp}`;
export const PROD_DOCKING_HOST_URL = 'https://app.opendome.xyz';

/**
 * @returns {string} Host origin without trailing slash
 */
export function resolveDockingHostUrl(env = process.env) {
 ...
 */
export async function exchangeDockingEnrollment(env = process.env) {
  ...
  try {
    ...
    return Response.json(
      { error: 'Docking host is unavailable', hostUrl },
      { status: 502 }
    );
  }
}
