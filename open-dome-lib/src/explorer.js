 ...
 */
export function buildPassExplorerLinks({
  toAddress,
  tokenId,
  mintTxHash,
  paymentTxHash,
  contractAddress = OPENDOME_PASS_ADDRESS,
  network = OPENDOME_PASS_NETWORK,
} = {}) {
  const contract = String(contractAddress || OPENDOME_PASS_ADDRESS);
  ...
  return {
    ...
    tokenInventoryUrl: owner
      ? `${BASESCAN}/token/${contract}?a=${owner}`
...
export function attachExplorerToPass(pass, { toAddress, mintTxHash, paymentTxHash, contractAddress } = {}) {
