...
export const X402_PAYMENT_CHAIN_KEYS = [
  'BASE',
  'ARB',
  'OP',
  'MATIC',
...
export const USDC_BASE = USDC_CHAINS.BASE.usdc;

export function normalizeUsdcChainKey(raw) {
  const key = String(raw || 'BASE')
    .trim()
    .toUpperCase()
    ...
    .replace(/^AVALANCHE$/, 'AVAX')
    .replace(/^SOLANA$/, 'SOL');
  return USDC_CHAINS[key] ? key : null;
}

export function getUsdcChain(raw = 'BASE') {
  const key = normalizeUsdcChainKey(raw) || 'BASE';
  return USDC_CHAINS[key];
}

export function listSendUsdcChains() {
 ...
 */
export function resolveUsdcRpcUrls(chain, env = process.env) {
  const cfg = typeof chain === 'string' ? getUsdcChain(chain) : chain;
  if (!cfg) return [...USDC_CHAINS.BASE.rpcs];

  const fromEnv =
    env?.[cfg.rpcEnv] || (cfg.key === 'BASE' ? env?.RPC_URL : null);
  const curated = Array.isArray(cfg.rpcs) && cfg.rpcs.length
    ? cfg.rpcs
    : [cfg.defaultRpc].filter(Boolean);
 ...
 */
export function resolveX402PaymentNetwork(raw = 'base') {
  const s = String(raw || 'base').trim().toLowerCase();
  if (['mainnet', 'ethereum', 'eth'].includes(s)) {
    const err = new Error(
      'Ethereum mainnet is not supported for x402 (gas too expensive). Use an L2 or Solana.',
    );
    err.status = 400;
    throw err;
  }
  if (s === 'monad') {
    const err = new Error(
      'Monad is not supported for x402 yet. Use Base, Arbitrum, Optimism, Polygon, Avalanche, or Solana.',
    );
    err.status = 400;
    throw err;
  }
  ...
  const key =
    normalizeUsdcChainKey(s) ||
    X402_UI_ALIASES[s] ||
    (USDC_CHAINS[String(raw || '').toUpperCase()]
      ? String(raw).toUpperCase()
      : null);

  if (!key || !X402_PAYMENT_CHAIN_KEYS.includes(key)) {
    const err = new Error(
      `Unsupported x402 network: ${raw}. Use Base, Arbitrum, Optimism, Polygon, Avalanche, or Solana.`,
    );
    err.status = 400;
    throw err;
  }
...
export function x402NetworkCaip(cfgOrKey) {
  const cfg =
    typeof cfgOrKey === 'string' ? getUsdcChain(cfgOrKey) : cfgOrKey;
  if (!cfg) return 'eip155:8453';
  if (cfg.key === 'SOL') return 'solana:mainnet';
  return `eip155:${cfg.chainId}`;
}

export function explorerTxUrl(cfgOrKey, txHash) {
