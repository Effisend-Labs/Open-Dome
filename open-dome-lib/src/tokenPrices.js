...
export const COINGECKO_IDS_BY_TICKER = {
  ETH: 'ethereum',
  SOL: 'solana',
  AVAX: 'avalanche-2',
...
export const FALLBACK_USD_BY_TICKER = {
  ETH: 3200,
  SOL: 145,
  AVAX: 35,
  POL: 0.5,
...
async function fetchWithRetries(url, options = {}, { retries = 3, delay = 1000, backoff = 2 } = {}) {
  let lastErr;
  let wait = delay;
 ...
 */
export async function fetchTokenUsdPrices(tickers, { signal } = {}) {
  const list = (tickers?.length ? tickers : Object.keys(COINGECKO_IDS_BY_TICKER))
    .map((t) => String(t || '').toUpperCase())
    .filter((t) => COINGECKO_IDS_BY_TICKER[t]);

  const uniqueTickers = [...new Set(list)];
  const ids = [...new Set(uniqueTickers.map((t) => COINGECKO_IDS_BY_TICKER[t]))];
  const url = `https://api.coingecko.com/api/v3/simple/price?ids=${ids.join(',')}&vs_currencies=usd`;

  const response = await fetchWithRetries(url, { method: 'GET', signal }, { retries: 3, delay: 1000, backoff: 2 });
...
export function priceForTicker(prices, ticker) {
...
export function resolveTokenPricesBaseUrl({ baseUrl } = {}) {
 ...
 */
export async function fetchCachedTokenUsdPrices(tickers, options = {}) {
  const { baseUrl, signal, hostRequest, allowDirectCoinGecko = false } = options;
  ...
  const origin = resolveTokenPricesBaseUrl({ baseUrl });
  if (origin) {
    const qs = tickers?.length
      ? `?tickers=${encodeURIComponent(tickers.join(','))}`
      : '';
    const res = await fetch(`${origin}/api/token-prices${qs}`, { method: 'GET', signal });
  ...
  if (allowDirectCoinGecko) {
    return fetchTokenUsdPrices(tickers, { signal });
  }
