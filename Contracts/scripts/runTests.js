...
async function main() {
    ...
    // Give user1 a ticket again (wait, maxPerWallet is 1, so since balance is 0, they can mint again!)
    await contract.connect(merchant).mint(user1.address, 1, 1, "0x");
    
    // Scan batch
    const tx = await contract.connect(scanner).scanPassBatch([user1.address, user2.address], [1, 2], [2, 1]); // user1 asks for 2 (fails), user2 asks for 1 (succeeds)
